...
const BINARY_REPO = "tokenharbor/thcode";
...
/**
 * Synchronous update — re-installs the wrapper via npm and re-downloads
 * the branded binary. Designed to be invoked by `thcode update`.
 */
export async function runUpdate({ binaryRefresher }) {
  ...
  const npmRes = spawnSync(
    ...
    ["i", "-g", `github:${WRAPPER_REPO}`],
    { stdio: "inherit" },
  );
  if (npmRes.status !== 0) {
    console.error("\nWrapper update failed. Try manually:");
    console.error(`  npm i -g github:${WRAPPER_REPO}`);
    process.exit(1);
  }
