...
async function downloadBranded() {
  ...
  if (!existsSync(extracted)) { console.error(`thcode: binary not found in ${asset}.`); return false; }
...
export function codewhaleBinaryPath() {
  ...
  // Fall back to npm-installed codewhale only if our binary is absent
  // (e.g., user disabled postinstall). Not preferred — it'll show
  // upstream branding — but better than crashing.
  return which("codewhale") || THCODE_BIN;
}

export async function ensureCodewhaleInstalled() {
  if (existsSync(THCODE_BIN) && existsSync(BINARY_TAG_FILE)) return;
  console.log("\nthcode: binary not found locally, downloading…");
  const ok = await downloadBranded();
  if (!ok) {
    console.error("\nthcode: install failed. See https://github.com/tokenharbor/thcode-tui/releases for manual download.");
    process.exit(1);
  }
}
