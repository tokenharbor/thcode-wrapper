...
async function downloadAndExtractThcode() {
  ...
  // Prefer system curl with built-in retry — node fetch on flaky
  // networks (China <-> GitHub objects edge) routinely gets reset
  // mid-handshake; curl --retry handles transient TLS resets +
  // follows redirects + supports IPv4-fallback heuristics out of the
  // box that node's undici doesn't replicate.
  const curlAvailable = which("curl");
  if (curlAvailable) {
    const r = spawnSync(
      "curl",
      [
    ...
    if (r.status !== 0) {
      console.error(
        "thcode: curl download failed (exit " + r.status + "). " +
        "Network can't reach github.com release objects. Falling back to upstream opencode.",
      );
      return false;
    }
  } else {
    // Final fallback to node fetch (no retry — rare on macOS / linux
    // where curl is preinstalled).
    try {
