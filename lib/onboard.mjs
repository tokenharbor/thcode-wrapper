...
function buildThcodeConfig(apiKey, models) {
  ...
  return {
    ...
    // Multi-agent role → model map.
    //
    // We route each role to a Token Harbor SMART BRAND (smart-thinking
    // for reasoning-heavy work, smart-fast for cheap I/O), NOT to a
    // hardcoded upstream like glm-5.1 or deepseek-v4-pro.
    //
    // Why brands beat hardcoded picks:
    //   1. The smart router behind each brand picks the best upstream
    //      live (Claude / GPT / DeepSeek / GLM / Kimi / Qwen / …) —
    //      thcode users don't have to track which is hot this month.
    //   2. When Token Harbor adds a new upstream (e.g. Claude 5),
    //      every existing thcode install picks it up immediately via
    //      the smart router — no wrapper update, no binary rebuild.
    //   3. Per-upstream outages fail over transparently inside the
    //      brand; the role-router doesn't see the churn.
    //   4. Power users can still override any individual role to a
    //      direct vendor id (deepseek-v4-pro, glm-5.1, …) via config.
    //
    // smart-thinking → planner/coder/reviewer: these turns benefit
    // from chain-of-thought + careful tool use; the brand routes to
    // reasoning-strong models.
    // smart-fast → fetcher/summarizer: heavy on I/O, cheap completions
    // are fine; brand routes to fast/cheap pool.
    roles: (() => {
      const pickBrand = (...needles) => {
        for (const needle of needles) {
          const m = models.find((x) => x.id === needle);
          if (m) return { provider: "tokenharbor", model: m.id };
        }
        // Smart brand not in this user's catalog — fall back to large.
        // Their gateway admin probably disabled smart-router; honor that.
        return { provider: "tokenharbor", model: largeId };
      };
      const thinking = pickBrand("tokenharbor-smart-thinking");
      const fast = pickBrand("tokenharbor-smart-fast");
      return {
        planner:    thinking,
        coder:      thinking,
        reviewer:   thinking,
