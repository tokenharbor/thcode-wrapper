...
function buildThcodeConfig(apiKey, models) {
  ...
  return {
    ...
    options: {
      // Auto-summarization at 80%/20% context fill truncates earlier
      // turns, which leads to "我不知道你说的'这'是什么" responses after
      // a noisy ls/grep dump. We'd rather lose tail of context than head.
      disable_auto_summarize: true,
    },
    tools: {
      ls: {
        // Default 1000 makes the agent refuse on common workspace roots
        // (~/Documents/TDI etc.) without ever surfacing real contents.
        // Bump it so the agent can enumerate normal-sized trees; the
        // glob/grep fallbacks still cover pathological dirs.
        max_items: 5000,
      },
    },
  };
