...
export async function runOnboarding({ configDir, dataDir, configFile, authFile }) {
  ...
  // Install omo-slim (oh-my-opencode-slim) — agent orchestrator
  // plugin that adds Todo Enforcer / Ralph Loop / async subagents /
  // curated MCPs. Routing config (which model each agent uses) is
  // pulled from Token Harbor's /api/thcode/routing so Season can
  // tune it centrally from /admin/thcode/routing without users
  // touching local files.
  await installOmoAndFetchRouting().catch((err) => {
    console.log(
